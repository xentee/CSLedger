...
create policy if not exists boards_owner on boards for all
  using (user_id = auth.uid());

alter table entries enable row level security;
create policy if not exists entries_board_owner on entries for all
  using (exists (select 1 from boards b where b.id = board_id and b.user_id = auth.uid()));

alter table subscriptions enable row level security;
create policy if not exists subs_owner on subscriptions for all
  using (user_id = auth.uid());
